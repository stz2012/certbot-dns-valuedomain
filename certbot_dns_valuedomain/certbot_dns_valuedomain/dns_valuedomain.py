...
from certbot_dns_valuedomain._version import __version__
...
class Authenticator(dns_common.DNSAuthenticator):
    ...
    @classmethod
    def add_parser_arguments(cls, add: Callable, default_propagation_seconds: int = DEFAULT_PROPAGATION_SECONDS):
...
class ValueDomainClient:
    ...
    def __init__(self, api_key: str, domain: str, timeout: int = DEFAULT_TIMEOUT, retry_count: int = DEFAULT_RETRY_COUNT):
        ...
        self.session.headers.update({
            'User-Agent': f'certbot-dns-valuedomain/{__version__}'  # ← バージョン使用
        })

    def add_txt_record(self, record_name: str, record_content: str, record_ttl: int = 60):
        """Add TXT record.
    ...
    def del_txt_record(self, record_name: str, record_content: str):
        """Delete TXT record.
    ...
    def _set_dns_records(self, records: list):
        """Set DNS records.
